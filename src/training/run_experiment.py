...
class TimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x_f = self.data[
            ...
            + self.output_seq_len
            + self.shift,
            self.input_forecast_features_list,
        ]
        y = self.data[
            idx
            + self.input_seq_len
...
for input_seq_len in [24, 48, 72]:
    for output_seq_len in [24, 48]:
        for shift in [0]:
            ...
            df_train, df_test = select_train_test(
                train_path="code/fields/field_f2", test_path="code/fields/field_f10"
            )
            dataloader_train, dataloader_test, scaler = set_data_loader(
                df_train,
                df_test,
                input_seq_len,
                output_seq_len,
            ...
            torch.save(
                model.state_dict(),
                f"weights/model_weights_{input_seq_len}_output_seq_len_{output_seq_len}_shift_{shift}.pth",
            )
